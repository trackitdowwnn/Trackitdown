...
comment on function public.match_alert_zones(uuid, int) is
  'SERVICE ROLE ONLY. The spotter-alert fan-out: returns { user_ids: [...], title, body } for one post and records a push_sends row per matched user in the SAME transaction, under pg_advisory_xact_lock(''notify_spotters:<post_id>''), so the cap cannot be raced. A user matches when an alert_zones row of theirs is enabled, is NOT the post owner, ST_DWithin(zone.point, post.last_seen_location, zone.radius_m) (geography => metres, GiST-assisted), SATISFIES THAT ALERT''S CRITERIA — make / model / colour / body_type compared lower(btrim(...)) on both sides, min_bounty_pence <= the post''s bounty (integer pence, GBP), last_seen_at inside recency_days; NULL on the zone side means ANY — and has not already been sent this post''s alert, and is under the rolling-24h cap. THE COPY IS BUILT HERE ON PURPOSE so its privacy properties are covered by npm run test:db rather than a second (Deno) test stack: the push carries the district-grain last_seen_locality ONLY (in the TITLE since 2026-09-22 — "Car stolen in Hemel Hempstead"; never street-grain last_seen_area), never the plate, never coordinates, and the body always ENDS with the don''t-approach clause (SECURITY_AND_TRUST §1); body_type/bounty/last_seen_at are read for MATCHING ONLY and are deliberately never interpolated into either line. Each interpolated value is bounded BEFORE assembly (descriptor 48 in the body, locality 40 in the title) rather than by truncating a finished sentence — the clause is last in the body, so an outer left() would let an owner pad `make` until the safety line fell off the end; the title carries no safety clause and so is bounded normally. Does not send.';
