 ...
 * WHY:   ⚠️ Deliberate override of "tabs get invitations, sheets fire on
 *        actions" (features/auth/README.md): a guest's Profile tab has
 *        nothing to show but the invitation, so the tap IS the action. Inbox
 *        joined the override on 2026-08-06, and the press half now lives in
 *        the shared useTabAuthGate so the two tabs cannot drift; My Cars keeps
 *        its invitation screen. One boundary —
 ...
 *        (iconUri contract); src/features/auth/gate/useTabAuthGate.ts (the
 *        shared press gate); src/features/auth/gate/useRequireAuth.ts.
 */

import { useMemo } from 'react';

import { type TabAuthGate, useAuthStanding, useTabAuthGate } from '@/features/auth';
...
export function useProfileTab(): ProfileTabState {
  ...
  // Hold-and-sheet: don't switch tabs — the sheet opens over where the guest
  // already is, and dismissal leaves them exactly there. Shared with the Inbox
  // tab, which runs the identical gate from the tabs layout.
  const listeners = useTabAuthGate({
    context: 'tab_profile',
    route: '/(tabs)/profile',
  });
